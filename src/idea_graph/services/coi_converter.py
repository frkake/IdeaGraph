...
class CoIConverter:
    ...
    CONVERSION_PROMPT_TEMPLATE = """You are an expert AI research analyst. Extract structured information from the following CoI-generated research idea.
...
## Instructions
Extract the following structured fields from the above text:
...
Focus on extracting concrete, actionable research information. If information is not clearly stated, make reasonable inferences based on the context."""
    ...
    def convert_to_proposal(self, coi_result: CoIResult) -> Proposal:
        ...
        proposal = Proposal(
            ...
            rationale=f"Generated by CoI-Agent from idea chain analysis. Trend: {coi_result.trend[:200] if coi_result.trend else 'N/A'}",
            research_trends=coi_result.trend or "Not available from CoI output",
            ...
            experiment=Experiment(
                ...
                failure_interpretation="If the proposed method does not outperform baselines, it may indicate that the key assumptions need revision or the approach requires further refinement.",
            ),
            grounding=Grounding(
